...
TOOSII-XD ULTRA Bot
Powered By Toosii Tech
© Toosii Tech 2024 - 2026

Contact
WhatsApp : https://wa.me/254748340864
...
async function writeExifImgAV (media, metadata) {
    let wMedia = await imageToWebp3(media)
    const tmpFileIn = path.join(tmpdir(), `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.webp`)
    const tmpFileOut = path.join(tmpdir(), `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.webp`)
    fs.writeFileSync(tmpFileIn, wMedia)
...
module.exports = { imageToWebp, imageToWebp3, videoToWebp, writeExifImg, writeExifImgAV, writeExifVid, writeExif }
